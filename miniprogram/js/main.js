...
export default class Main {
  ...
  render() {
    ...
    this.gameinfo.renderGameAddr(ctx, addr)
    this.gameinfo.renderGameCId(ctx, cid)

    // 游戏结束停止帧循环
    if ( databus.gameOver ) {
      this.gameinfo.renderGameRecharge(ctx)
  ...
  update() {
    ...
    if (databus.frame % databusFrame === 0 ) {
      this.player.shoot()
